...
def main(MerakiTimer: func.TimerRequest) -> None:
    ...
    # writing function to obtain org ID via linking Meraki ORG name
    ...
    for i in tagsnetwork:
        ...
        if i['tags'] is None:
            ...
            if firmwarecompliance == True:
                ...
                logging.info("firmware is compliant, continuing")
            ...
            if (storage_container.status_code < 200 and vwan_hub_info.status_code > 201):

                logging.error("Failed creating container for storage account")
            ...
            for element in vwan_config_file:
                ...
                if element['vpnSiteConfiguration']['Name'] == netname2: # match all VPN config based upon vpnsite name and merakidashboard name

                    logging.info('Found your match')
            ...
            logging.info(azureinstance0)

            logging.info(azureinstance1)

            logging.info(azureconnectedsubnets)

            logging.info("look above")

 

            specifictag = re.findall(r'[v]+[W]+[A]+[N]+[-]+[0-999]', str(nettag)) # matches all primary vWAN tags
            ...
            if not any(site['name'] == netname for site in merakivpns):

                logging.info(type(addpsk))
